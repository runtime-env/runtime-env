...
export const createGeneratorForJSONSchema: CreateGenerator = async ({
  ...
  return <CreateGeneratorReturnType>{
    ...
    generateTs: async () => {
      ...
      ensureNoDefaultKeyword(envSchemaFileJSON);
...
const ensureNoDefaultKeyword = (schema: any, path: string = "data"): void => {
  ...
  if (Array.isArray(schema)) {
    schema.forEach((item, index) => {
      ensureNoDefaultKeyword(item, `${path}/${index}`);
    });
    return;
  }

  if ("default" in schema) {
    throwError(`schema is invalid: ${path}/default is prohibited`);
  }

  Object.entries(schema).forEach(([key, value]) => {
    if (typeof value === "object" && value !== null) {
      ensureNoDefaultKeyword(value, `${path}/${key}`);
    }
  });
};
