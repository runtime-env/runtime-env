...
- **THEN** they can provide a nested configuration object for each sub-command (`gen-ts`, `gen-js`, `interpolate`).
...
- **AND** the `interpolate` sub-command SHALL only be executed if an `interpolate` object is present.
- **AND** the options within each object correspond to the CLI arguments for that sub-command (e.g., `outputFile`, `envFile`).
- **AND** `envFile` is a valid option only for `gen-js` and `interpolate` objects.
...
- **AND** the package.json of the plugin SHALL specify `vite: ">=5"` in its `peerDependencies`.
...
- **GIVEN** the `@runtime-env/vite-plugin` plugin is configured in `vite.config.ts` with `{'gen-ts': {...}, 'gen-js': {...}, 'interpolate': {...}}`.
- **WHEN** the user runs `vite` (or `npm run dev`).
- **THEN** the plugin automatically runs the configured commands (`gen-ts`, `gen-js`, `interpolate`) in watch mode.
- **AND** if the `interpolate` object is present, the Vite dev server serves the correctly interpolated `index.html` in memory.
...
- **AND** the plugin SHALL NOT run `gen-js` or `interpolate`, preserving the "build once, deploy anywhere" principle.
...
- **AND** the `@runtime-env/vite-plugin` plugin is configured with `{'gen-js': {...}, 'interpolate': {...}}`.
...
- **THEN** the plugin provides a setup function that runs `gen-ts` and `gen-js` (if a `gen-ts` or `gen-js` object is present in the config) before any tests are executed.
- **AND** the `package.json` `test` script is simply `"test": "vitest"`.
