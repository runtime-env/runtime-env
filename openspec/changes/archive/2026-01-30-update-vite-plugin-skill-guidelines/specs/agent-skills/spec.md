...
- **AND** if an environment variable is parsed or transformed (e.g., `parseInt(env.VAR)`, `JSON.parse(env.VAR)`, or boolean checks like `env.VAR === 'true'`) in the source code, it SHOULD be defined as a `string` in the schema to minimize codebase changes
