...
- [ ] Create `cypress/support/commands.js` for custom commands (spawn/kill utilities)
...
- [ ] Test: Spawn dev server using cy.exec('npm run dev > dev.log 2>&1 & echo $!') and capture PID
- [ ] Test: Wait for server ready (poll localhost:5173)
...
- [ ] Test: Terminate dev server using cy.exec('kill <pid>')

### comprehensive-webpack: cypress/e2e/dev.cy.js

- [ ] Test: Spawn dev server using cy.exec('npm run dev > dev.log 2>&1 & echo $!') and capture PID
- [ ] Test: Wait for server ready (poll localhost:8080)
...
- [ ] Test: Run `npm run build` via cy.exec() WITHOUT .env (prerequisite)
- [ ] Test: Create .env AFTER build using cy.writeFile()
- [ ] Test: Spawn preview server using cy.exec('npm run preview > preview.log 2>&1 & echo $!') and capture PID
- [ ] Test: Wait for server ready (poll localhost:4173)
...
- [ ] Test: Terminate preview server using cy.exec('kill <pid>')
- [ ] Test: Update .env file with new value
- [ ] Test: Spawn preview server again (NO rebuild)
...
- [ ] Add "Test examples/comprehensive-vite (dev)" step (no git clean on first step)
- [ ] Add "Test examples/comprehensive-vite (test)" step (with `git clean -xdf` in example dir)
- [ ] Add "Test examples/comprehensive-vite (preview)" step (with `git clean -xdf` in example dir)
- [ ] Add "Test examples/comprehensive-vite (docker)" step (with `git clean -xdf` in example dir)

### comprehensive-webpack CI Steps

- [ ] Add "Test examples/comprehensive-webpack (dev)" step (no git clean on first step)
- [ ] Add "Test examples/comprehensive-webpack (test)" step (with `git clean -xdf` in example dir)
- [ ] Add "Test examples/comprehensive-webpack (docker)" step (with `git clean -xdf` in example dir)

### CI Configuration

- [ ] Ensure tarball installation from `../../packages/cli/runtime-env-cli-test.tgz` (same job, no artifact download)
- [ ] Ensure `git clean -xdf` runs inside example directory between test modes (except first step)
- [ ] Ensure builds run WITHOUT .env file
- [ ] Ensure Docker containers are cleaned up after tests
- [ ] Verify steps run in same job after pack step
