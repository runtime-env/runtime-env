# Capability: Next.js Plugin

The `@runtime-env/next-plugin` provides zero-config integration for Next.js applications.
...
- **WHEN** the Next.js server starts (including during `next build`)
...
- **THEN** NO instances of the `any` type SHALL be found in the `src/` directory
