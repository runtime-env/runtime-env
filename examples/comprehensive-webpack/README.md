...
- **Progressive Web App (PWA)** support with service worker cache busting
...
- ✅ Service worker with runtime-env.js cache busting
...
This generates runtime-env.js, interpolates index.html, patches service worker, and serves static files.
...
- **Workbox Plugin**: Generates service worker with runtime-env.js precaching
- **Dev Server**: Serves static files from `public/` directory

## PWA Support

Service worker is generated by `workbox-webpack-plugin` with cache busting:

1. Workbox generates `service-worker.js` with placeholder revision for `runtime-env.js`
2. `scripts/patch-runtime-env-revision.cjs` calculates MD5 hash and patches service worker
3. Service worker precaches runtime-env.js with correct revision hash
...
- Patches service worker with runtime-env.js revision
...
├── scripts/
│   └── patch-runtime-env-revision.cjs # Service worker patcher
├── dist/
...
4. **patch-runtime-env-revision**: Patches service worker with MD5 hash
...
Run `npm run preview` to regenerate service worker with patched revision.
